...
```{r simple-plot, fig.height = 6, fig.width=12, fig.align = 'center', message = F,echo=FALSE}
library(UsingR)
library(dplyr)
library(manipulate)
...
g <- g + geom_abline(intercept = coef(lm)[1], slope = coef(lm)[2], size = 3, colour = grey(.5))
g <-g+ labs(title = paste("Adj R2 = ",signif(summary(lm)$adj.r.squared, 5),
